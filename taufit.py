...
def fit_drw(x, y, yerr, nburn=500, nsamp=2000, bounds='default', color="#ff7f0e", plot=True, verbose=True):
    ...
    if bounds == 'default':
        ...
        bounds = dict(log_a=(-15.0, 5.0), log_c=(cmin, cmax))
                
    kernel = terms.RealTerm(log_a=0, log_c=np.mean([cmin,cmax]), bounds=bounds)
    ...
    if plot:
        ...
        axs[0,0].set_ylabel("Power ($\mathrm{ppm}^2 / day^{-1}$)",fontsize=18)
        ...
        # Plot ACF
        axs[1,1].acorr(res2-np.mean(res2), maxlags=maxlag, lw=2)
        # White noise
        wnoise_upper = 1.96/np.sqrt(len(x))
        wnoise_lower = -1.96/np.sqrt(len(x))
        axs[1,1].fill_between([0, maxlag], wnoise_upper, wnoise_lower, facecolor='grey')
...
def fit_complex(x, y, yerr, twoJ=1, nburn=500, nsamp=2000, bounds=None, color="#ff7f0e", plot=True, verbose=True):
        
    # Sort data
    ind = np.argsort(x)
    x = x[ind]; y = y[ind]; yerr = yerr[ind]
    
    kernel = terms.ComplexTerm(log_a=1, log_b=0.1, log_c=0.1, log_d=0.1, bounds=bounds)
    for j in range(1,twoJ):
        kernel += terms.ComplexTerm(log_a=1, log_b=0.1, log_c=0.1, log_d=0.1, bounds=bounds)
    ...
    # Noise level
    noise_level = 2.0*np.median(np.diff(x.value))*np.mean(yerr.value**2)
    log_tau = np.log10(1/np.exp(samples[:,2]))
    ...
    if plot:
        ...
        # tau_DRW posterior distribution
        axs[1,0].hist(log_tau, color=color, alpha=0.8, fill=None, histtype='step', lw=3, bins=50, label=r'posterior distribution')
        ylim = axs[1,0].get_ylim()
        ...
        axs[1,0].set_ylim(ylim)
        axs[1,0].set_xlim(np.min(log_tau), np.max(log_tau))
        axs[1,0].set_xlabel(r'$\log_{10} \tau_{c} (days)$',fontsize=18)
        axs[1,0].set_ylabel('Count',fontsize=18)
        axs[1,0].tick_params(labelsize=18)
